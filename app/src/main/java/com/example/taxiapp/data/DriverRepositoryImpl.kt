...
class DriverRepositoryImpl(
    ...
    private val TAG = "DriverRepositoryImpl"
    private val databaseRef = realtimeDB.getReference(Constants.DRIVERS)

    override fun registerDriver(driver: Driver, password: String): Resource<Boolean> {
    ...
    override fun saveDriverData(driver: Driver): Resource<Boolean> {
        var resource : Resource<Boolean> = Resource.Success(true)
        val userId = firebaseAuth.currentUser?.uid

        if (userId != null) {
            databaseRef.child(userId).setValue(driver.copy(id = userId))
    ...
    override fun getDriver(id: String, callback: DriverCallback) {
        databaseRef.child(id).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                if (dataSnapshot.exists()) {
                    ...
                    Log.d("HERE!!", driver.toString())
                    if (driver != null) {
                        callback.onDriverFound(Resource.Success(driver))
                    } else {
                        callback.onDriverNotFound(Resource.Error("User is not found.", null))
                    }
                } else {
                    callback.onDriverNotFound(Resource.Error("User not found.", null))
                }
            }

            override fun onCancelled(databaseError: DatabaseError) {
                callback.onDriverNotFound(Resource.Error(databaseError.message.toString(), null))
            }
        })
    }
